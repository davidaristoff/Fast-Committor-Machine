%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%% Fast Committor Machine for double-well potential example %%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for repeat = 1:repeats   %repeat the experiments
    ...
    for iter = 1:iters
        ...
        M = get_Mahalanobis_matrix(X,dq,S,N,d,samples);
...
q = @(Z) k(Z,X(S,:))*(XinI(S).*eta) ...
            -k(Z,Y(S,:))*(YinI(S).*eta);

%construct gradient of committor (z = row vector)
dq = @(z) dk(z,X(S,:))*(XinI(S).*eta) ...
            -dk(z,Y(S,:))*(YinI(S).*eta);
...
function M = get_Mahalanobis_matrix(X,dq,S,N,d,samples)

%get samples away from selected ones
ind = randi(N,[samples 1]); ind = ind(~ismember(ind,S));
